...
- сценариев может быть несколько, где у каждого может быть параметр:
     - зациклить.
     - выполнять до победного.
     - строгое выполнение.
- в параметре "цикличность" установить через сколько повторять N {годов/месяцев/недель/дней/часов/минут} и сколько N раз.
- параметр "выполнять до победного" утверждает неопределённый срок выполнения и может влезать во временной промежуток других целей. 
...
- распределяем сценарии цели на временной шкале: 
     - промежуток: начинать с {даты/день недели} со временем (час, минута).
